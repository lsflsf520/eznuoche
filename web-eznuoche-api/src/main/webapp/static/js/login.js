...
function sendCode2(thisBtn) {
	...
	if (iscode) {
		...
		iscode = false;
		Kino.ajax(chexianapi + '/findpwd/sms/send.do?phone=' + mobile
				+ '&pkey=' + uuid, successfn, obj, shibai);
	}
}
					...
					if (zheyanz()) {
						...
						(function() {
							function successfn(response, status, xhr) {
								...
								Kino.ajax(chexianapi + '/user/resetPwd.do',
										successfn, obj);
							}
							var obj = {
								timeoutFn : function() {
							...
							Kino.ajax(chexianapi + '/user/genSecCode.do?phone='
									+ phone_val + '&_code_=' + code_val
									+ '&pkey=' + uuid, successfn, obj);
						})();
